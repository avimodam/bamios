  ...
  var Bamios = /*#__PURE__*/function () {
    ...
    _createClass(Bamios, [{
      ...
      value: function initEvents() {
        var _this = this;
        var mouseClickedProgressBar = false;
        var mouseEnteredProgressBar = false;
        ...
        this.audio.ontimeupdate = function () {
          _this.currentTimeText.innerHTML = Bamios.toTimeString(_this.audio.currentTime);
          var currentPercent = _this.audio.currentTime / _this.audio.duration * 100 + '%';
          _this.playProgress.style.width = currentPercent;
          if (!mouseClickedProgressBar) {
            _this.progressHandle.style.transform = 'translate(' + (_this.playProgress.getBoundingClientRect().width - 8) + 'px, -50%)';
          }
        };
        ...
        this.playBtn.addEventListener('click', function (playBtnEvent) {
        ...
        this.progressBar.addEventListener('mousedown', function (progressBarEvent) {
          mouseClickedProgressBar = true;
          _this.isAdjustingTime = true;
          _this.pause();
          var offsetX = progressBarEvent.clientX - _this.progressBar.getBoundingClientRect().left;
          if (offsetX >= 0 && offsetX <= _this.progressBar.getBoundingClientRect().width) {
            var second = offsetX / _this.progressBar.getBoundingClientRect().width * _this.audio.duration;
            _this.setCurrentTime(second);
            _this.progressHandle.style.transform = 'translate(' + (offsetX - 8) + 'px, -50%)';
          }
        });
        window.addEventListener('mouseup', function () {
          ...
          if (!mouseEnteredProgressBar) {
            _this.progressHandle.style.transition = 'opacity 0.6s ease-in-out';
            _this.progressHandle.style.opacity = 0;
          }
          if (!_this.muted) {
        ...
        this.progressBar.addEventListener('mouseenter', function () {
          ...
          _this.progressHandle.style.display = 'block';
          _this.progressHandle.style.transition = 'opacity 0.15s ease-in-out';
          _this.progressHandle.style.opacity = 1;
        });
        this.progressBar.addEventListener('mouseleave', function () {
          mouseEnteredProgressBar = false;
          if (!mouseClickedProgressBar) {
            _this.progressHandle.style.display = 'none';
            _this.progressHandle.style.transition = 'opacity 0.6s ease-in-out';
            _this.progressHandle.style.opacity = 0;
          }
        });
        window.addEventListener('mousemove', function (progressBarEvent) {
          if (mouseClickedProgressBar) {
        ...
        this.volumeBtn.addEventListener('click', function (volumeBtnEvent) {
          if (_this.audio.volume > 0) {
            if (!volumeBtnEvent.currentTarget.classList.contains('muted')) {
              ...
              _this.volumeSlider.style.width = '0%';
              _this.volumeHandle.style.transform = 'translate(-8px, -50%)';
              _this.audio.muted = true;
              _this.muted = true;
            } else {
              volumeBtnEvent.currentTarget.classList.remove('muted');
              volumeBtnEvent.currentTarget.innerHTML = Bamios.volumeIcon();
              _this.volumeSlider.style.width = _this.audio.volume * 100 + '%';
              _this.volumeHandle.style.transform = 'translate(' + (_this.audio.volume * 80 - 8) + 'px, -50%)';
              _this.audio.muted = false;
              _this.muted = false;
            }
        ...
        this.volumeCtl.addEventListener('mouseenter', function () {
          ...
          if (window.innerWidth > 520) {
            _this.volumePanel.style.width = '80px';
            _this.volumeCtl.style.paddingLeft = '15px';
            _this.volumeCtl.style.background = 'rgba(0, 0, 0, 0.04)';
            _this.volumePanelContainer.style.marginRight = '10px';
            _this.volumePanel.style.opacity = 1;
            setTimeout(function () {
              if (mouseHoverVolumeCtl && !_this.audio.muted) {
                _this.volumeHandle.style.transform = 'translate(' + (_this.audio.volume * 80 - 8) + 'px, -50%)';
              }
            }, 150);
          }
        ...
        this.volumeCtl.addEventListener('mouseleave', function () {
          if (!mouseClickedVolumeWrapper) {
            ...
            _this.volumeCtl.style.paddingLeft = 0;
            _this.volumeCtl.style.background = 'transparent';
            _this.volumePanelContainer.style.marginRight = 0;
            _this.volumePanel.style.opacity = 0;
            _this.volumeHandle.style.opacity = 0;
            _this.volumeHandle.style.display = 'none';
          }
        });
        this.volumeCtl.addEventListener('mouseenter', function () {
        ...
        this.volumePanelContainer.addEventListener('mousedown', function (volumePanelContainerEvent) {
          ...
          var offsetX = volumePanelContainerEvent.clientX - _this.volumePanelContainer.getBoundingClientRect().left;
          if (offsetX >= 0 && offsetX <= 80) {
            var volume = offsetX / 80;
            if (volume < 0) volume = 0;
            if (volume > 1) volume = 1;
            _this.audio.volume = volume;
            _this.volumeSlider.style.width = volume * 100 + '%';
            _this.volumeHandle.style.transform = 'translate(' + (offsetX - 8) + 'px, -50%)';
            if (volume <= 0) {
              _this.volumeBtn.innerHTML = Bamios.volumeOffIcon();
              _this.audio.muted = true;
              _this.muted = true;
            } else if (volume <= 0.6) {
              _this.volumeBtn.innerHTML = Bamios.volumeLowIcon();
              _this.audio.muted = false;
              _this.muted = false;
            } else {
              _this.volumeBtn.innerHTML = Bamios.volumeIcon();
              _this.audio.muted = false;
              _this.muted = false;
            }
          }
        ...
        window.addEventListener('mousemove', function (volumePanelContainerEvent) {
          if (mouseClickedVolumeWrapper) {
            ...
            if (offsetX <= 0) {
              ...
              _this.volumeSlider.style.width = 0;
              _this.volumeHandle.style.transform = 'translate(-8px, -50%)';
            } else if (offsetX > 0 && offsetX <= _this.volumePanelContainer.getBoundingClientRect().width) {
              _this.audio.volume = volume;
              _this.volumeSlider.style.width = volume * 100 + '%';
              _this.volumeHandle.style.transform = 'translate(' + (offsetX - 8) + 'px, -50%)';
            } else {
              ...
              _this.volumeHandle.style.transform = 'translate(' + (80 - 8) + 'px, -50%)';
            }
            if (volume <= 0) {
              _this.volumeBtn.innerHTML = Bamios.volumeOffIcon();
              _this.audio.muted = true;
        ...
        this.volumePanelContainer.addEventListener('mouseenter', function () {
          ...
          _this.volumeHandle.style.opacity = 1;
          _this.volumeHandle.style.display = 'block';
        });
